...
def blend_hex(hex1, hex2, ratio=0.25):
    """Blend two hex colors with the given ratio of hex2 (0.0 to 1.0)."""
    r1, g1, b1 = hex_to_rgb(hex1)
    r2, g2, b2 = hex_to_rgb(hex2)
    r = int(round(r1 * (1.0 - ratio) + r2 * ratio))
    g = int(round(g1 * (1.0 - ratio) + g2 * ratio))
    b = int(round(b1 * (1.0 - ratio) + b2 * ratio))
...
"""
        ...
        # 2a. Foot SSH Terminal ssh-colors.ini & 2b. ~/.cache/wal/ssh-colors.sh & 2c. active-ssh.kdl
        ssh_bg = blend_hex(bg, primary_accent, ratio=0.25)
        ssh_cursor = primary_accent
        ssh_bg_hex = ssh_bg.lstrip("#")
        accent_hex = primary_accent.lstrip("#")

        foot_ssh_content = f"""[colors-dark]
alpha=0.95
background={ssh_bg_hex}
foreground={fg_hex}
cursor={accent_hex} {ssh_bg_hex}
selection-foreground={ssh_bg_hex}
selection-background={accent_hex}
urls={accent_hex}
...
bright7={c15.lstrip("#")}
"""
        with open(os.path.join(foot_dir, "ssh-colors.ini"), "w") as f:
            f.write(foot_ssh_content)

        wal_dir = os.path.expanduser("~/.cache/wal")
        os.makedirs(wal_dir, exist_ok=True)
        with open(os.path.join(wal_dir, "ssh-colors.sh"), "w") as f:
            f.write(f"""#!/usr/bin/env bash
# Auto-generated by pywal-cursor.py for dynamic SSH theming
export SSH_THEME_BG="{ssh_bg}"
export SSH_THEME_FG="{fg}"
export SSH_THEME_CURSOR="{ssh_cursor}"
export SSH_THEME_ACCENT="{primary_accent}"
""")

        niri_themes_dir = os.path.expanduser("~/.config/niri/themes")
        os.makedirs(niri_themes_dir, exist_ok=True)
        active_ssh_kdl = f"""// SSH Terminal Window Styling (Dynamic Pywal)
window-rule {{
    match app-id=r#"^foot\\.ssh.*"#
    border {{
        on
        width 2
        active-color "{primary_accent}"
        inactive-color "{ssh_bg}"
    }}
    shadow {{
        on
        softness 30
...
"""
        with open(os.path.join(niri_themes_dir, "active-ssh.kdl"), "w") as f:
            f.write(active_ssh_kdl)
...
"""
        ...
        if os.path.exists(niri_config):
            with open(niri_config, "r") as f:
                content = f.read()
            if 'include "./themes/active-ssh.kdl"' not in content:
                content = re.sub(
                    r'(include "\./themes/active-animations\.kdl"\n)',
                    r'\1include "./themes/active-ssh.kdl"\n',
                    content
                )
